...
class MockHsmSigner:
    def sign(self, _handle=None, _data=None, _mechanism=None):
...
class TestRemoteSigner(unittest.TestCase):
    ...
    def vote_test(self, req, got):
        ...
        rs = ValidateSigner(
            self.TEST_CONFIG, ratchet=MockChainRatchet(), subsigner=MockHsmSigner()
        )
    ...
    def test_identifies_invalid_block_preamble(self):
        with self.assertRaises(Exception):
            rs = ValidateSigner(
                self.TEST_CONFIG, ratchet=MockChainRatchet(0, 0), hsm=MockHsmSigner()
            )
    ...
    def test_list_sigreqs(self):
        for req in valid_sig_reqs:
            ...
            rs = ValidateSigner(
                self.TEST_CONFIG,
                ratchet=MockChainRatchet(level=got.level - 1, round=0),
                subsigner=MockHsmSigner(),
            )
            ...
            if got.round > 0:
                rs = ValidateSigner(
                    self.TEST_CONFIG,
                    ratchet=MockChainRatchet(level=got.level, round=got.round - 1),
                    subsigner=MockHsmSigner(),
                )
                self.assertEqual(rs.sign(7, SignatureReq(req[4])), SIGNED_BLOCK)
            ...
            with self.assertRaises(Exception):
                rs = ValidateSigner(
                    self.TEST_CONFIG,
                    ratchet=MockChainRatchet(got.level, got.round),
                    subsigner=MockHsmSigner(),
                )
